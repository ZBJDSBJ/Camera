...
package com.lost.zou.camera.common.view.cropper;
...
import com.lost.zou.camera.common.view.cropper.cropwindow.edge.Edge;
import com.lost.zou.camera.common.view.cropper.cropwindow.handle.Handle;
import com.lost.zou.camera.common.view.cropper.util.AspectRatioUtil;
import com.lost.zou.camera.common.view.cropper.util.HandleUtil;
import com.lost.zou.camera.common.view.cropper.util.PaintUtil;
import com.lost.zou.camera.common.view.photoview.PhotoView;
...
public class CropImageView extends PhotoView {
    ...
    public void rotateView() {
        ...
        //旋转图片动作
        matrix.reset();
        matrix.postRotate(toDegrees, getWidth() / 2, getHeight() / 2);

        Log.i("zou", "rotateView    getWidth() / 2= " + getWidth() / 2 + " getHeight() / 2 = " + getHeight() / 2
                + " mAspectRatioX= " + mAspectRatioX + " mAspectRatioY = " + mAspectRatioY);

        //创建新图片
        Bitmap originalBitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
        BitmapDrawable bmd = new BitmapDrawable(originalBitmap);
        this.setAdjustViewBounds(true);
        this.setImageDrawable(bmd);
...
//        rotate();
    ...
    public Bitmap getCroppedImage() {
        ...
        Log.i("zou", "cropWidth= " + cropWidth + " cropHeight = " + cropHeight + "\n cropX= " + cropX + " cropY = " + cropY);

        // Crop the subset from the original Bitmap.
        return Bitmap.createBitmap(originalBitmap,
                (int) Math.abs(cropX),
                (int) Math.abs(cropY),
                (int) Math.abs(cropWidth),
                (int) Math.abs(cropHeight));
    }
    ...
    private void initCropWindowWithFixedAspectRatio(@NonNull RectF bitmapRect) {
        ...
        if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > getTargetAspectRatio()) {
            Log.i("zou", "initCropWindowWithFixedAspectRatio " + bitmapRect.right);
            ...
            Log.i("zou", "bitmapRect.centerX= " + bitmapRect.centerX() + " bitmapRect.centerY()= " + bitmapRect.centerY()
                    + " cropHeight = " + cropHeight + " bitmapRect.left  = " + bitmapRect.left + " bitmapRect.right = " + bitmapRect.right);
            Edge.LEFT.setCoordinate((bitmapRect.right - 300) / 2f);
            Edge.TOP.setCoordinate(bitmapRect.centerY() - cropHeight / 2f + (cropHeight - 300) / 2f);
            Edge.RIGHT.setCoordinate(bitmapRect.right - (bitmapRect.right - 300) / 2f);
            Edge.BOTTOM.setCoordinate(bitmapRect.centerY() + cropHeight / 2f - (cropHeight - 300) / 2f);
        }
    }
     ...
     */
    private Handle onActionDown(float x, float y) {
